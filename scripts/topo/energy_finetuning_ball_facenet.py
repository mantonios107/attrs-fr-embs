...
from sklearn.metrics.pairwise import euclidean_distances, cosine_distances, paired_cosine_distances, paired_euclidean_distances
    ...
    for folder in folders:
        ...
        for p in tqdm(os.listdir(embeddings_path+"/")): # iterate over identities
            if p.split(".")[1] == "npy":
                ...
                d_avg = np.mean(paired_euclidean_distances(embeddings[ids1,:],embeddings[ids2,:]))
                ...
                for idx_dim,dim in enumerate(attributes):
                    ...
                    for eps in epsilon:
                        knng = RadiusNeighborsTransformer(radius=eps*d_avg, mode='connectivity', metric = "euclidean").fit_transform(embs_pc)
